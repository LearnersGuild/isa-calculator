...
test('src/dates', t => {
  t.test('lastPossibleExitDate', tt => {
    ...
    tt.test('returns Friday of (startDate + 42 weeks) if both break weeks are encountered', ttt => {
      ttt.plan(1)
      const exit = lastPossibleExitDate(new Date('2017-05-08'))
